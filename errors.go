...
type PairRedistributerError struct {
	msg string
}

func newPairRedistributorError(errMsg string) PairRedistributerError {
	return PairRedistributerError{
		msg: fmt.Sprintf("concurrent map: failing pair redistribution: %s", errMsg),
	}
}

func (pre PairRedistributerError) Error() string {
	return pre.msg
}
